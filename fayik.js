...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////
client1.on("ready", () => {
  client1.channels.cache.get('856244279151820830').join();
})
client2.on("ready", () => {
  client2.channels.cache.get('856244279151820830').join();
})
client3.on("ready", () => {
  client3.channels.cache.get('856244279151820830').join();
})
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.members.ban(entry.executor.id, {reason: "Specy System | İzinsiz kullanıcı yasaklama"}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.members.ban(yetkili.id, {reason: "Specy System | izinsiz kullanıcı Kickleme!"}).catch(e => { })	
...
const entry = await member.guild
  ...
  member.guild.members.ban(entry.executor.id, {reason: "Specy System | İzinsiz Bot Ekleme!"}).catch(e => { })	
  member.guild.members.ban(member.id, {reason: "Specy System | Bot Koruma Sistemi!"}).catch(e => { })	
...
client1.on('guildUpdate', async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, {reason: "Specy System | URL Koruma Sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: `Specy System | Sunucuyu izinsiz güncellemek.` }).catch(e => { })	
  const moment = require('moment');
  moment.locale('tr');
...
client2.on("roleDelete", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `Specy System | İzinsiz rol silme!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `Specy System | İzinsiz rol oluşturma!` }).catch(e => { })	
  role.delete({ reason: "Specy System | Rol Koruma Sistemi" }).catch(e => { })	
...
client2.on("roleUpdate", async (oldRole, newRole) => {
  ...
  let roleNames = [];
  await cur.roles.cache.forEach(role => roleNames.push(role.name));
  if (!utils.get("ohal").value()) {
      const model = await models.findOne({ _id: cur.user.id });
      if (!model) {
          let saver = await models({ _id: cur.user.id, roles: roleNames });
          await saver.save();
      } else {
          if (key === '$add') await models.updateOne({ _id: executor.user.id }, { $push: { roles: entry.changes[0].new[0].name } });
          if (key === '$remove') await models.updateOne({ _id: executor.user.id }, { $pull: { roles: entry.changes[0].new[0].name } });
          console.log(`${entry.executor.username} => [${entry.changes[0].key}] ${entry.target.username} : ${entry.changes[0].new[0].name}`, "mngdb");
      };
  };
  ...
  newRole.guild.members.ban(entry.executor.id, { reason: `Specy System | İzinsiz Rol Güncelleme!` }).catch(e => { })	
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Specy System | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "Specy System | Kanal Korum Sistemi!" }).then(async kanal => {
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Specy System | İzinsiz Kanal Oluşturma!` }).catch(e => { })
  channel.delete({reason: "Specy System | Kanal Koruma Sistemi!"}).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  newChannel.members.ban(member.id, { reason: `Specy System | İzinsiz Kanal Güncellemek!` }).catch(e => { })
...
client3.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: "Specy System | Webhook Silindi!"})).catch(err => { });
  channel.guild.members.ban(entry.executor.id, {reason: "Specy System | İzinsiz Webhook Açmak!"}).catch(err => { });
